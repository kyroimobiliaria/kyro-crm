...
// caches
const db = { leads: [], imoveis: [], ligacoes: [], corretores: [] };
...
document.getElementById('auth-cadastrar').addEventListener('click', async () => {
  const senha = v('auth-senha');
  if (senha.length < 6) return authMsg('A senha precisa ter ao menos 6 caracteres.', 'var(--red)');
  const { error } = await sb.auth.signUp({ email: v('auth-email'), password: senha });
  if (error) authMsg(error.message, 'var(--red)');
  else authMsg('Conta criada! Se pediu confirmação, veja seu e-mail e clique em "Entrar".', 'var(--gold)');
});
...
async function showView(name) {
  ...
  else if (name === 'acomp') { await carregarLeads(); await carregarLigacoes(); renderAcomp(); }
...
async function carregarLigacoes(all = false) {
  let q = sb.from('ligacoes').select('*');
  if (!all && ME && ME.role === 'corretor' && !isDemo()) q = q.eq('corretor_id', UID);
  const { data, error } = await q;
  if (error) return toast(error.message);
  db.ligacoes = data || [];
...
async function carregarTudo() {
  await Promise.all([carregarLeads(true), carregarLigacoes(true), carregarImoveis(), carregarCorretores()]);
}
...
function renderAcomp() {
  ...
  const f = db.leads;
  document.querySelector('#acomp-tabela tbody').innerHTML = f.map((l) => {
    const total = db.ligacoes.filter((x) => x.lead_id === l.id).length;
    const squares = dias.map((d) => {
      const ds = ymd(d);
      const on = db.ligacoes.some((x) => x.lead_id === l.id && x.data === ds);
      return `<td style="text-align:center"><span class="sq ${on ? 'on' : ''}" onclick="toggleLigacao('${l.id}','${ds}')"></span></td>`;
    }).join('');
    return `<tr><td>${esc(l.nome)}</td>${squares}<td style="text-align:center;font-weight:700;color:var(--gold)">${total}</td></tr>`;
  }).join('');
  document.getElementById('acomp-vazio').style.display = f.length ? 'none' : 'block';
}
async function toggleLigacao(leadId, dateStr) {
  const ex = db.ligacoes.find((x) => x.lead_id === leadId && x.data === dateStr);
  if (ex) {
    const { error } = await sb.from('ligacoes').delete().eq('id', ex.id);
    if (error) return toast(error.message);
  } else {
    const { error } = await sb.from('ligacoes').insert({ lead_id: leadId, corretor_id: isDemo() ? null : UID, data: dateStr });
    if (error) return toast(error.message);
  }
...
function renderDashboard() {
  ...
  const cards = [
    ...
    { num: frio, lbl: '🧊 Frios' },
    { num: db.ligacoes.length, lbl: 'Ligações registradas' },
  ...
  const porCorretor = db.corretores.map((c) => {
    ...
    return {
      nome: c.nome || c.email,
      leads: meus.length,
      ligacoes: db.ligacoes.filter((x) => x.corretor_id === c.id).length,
